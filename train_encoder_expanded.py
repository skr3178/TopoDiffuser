...
def main():
    ...
    # Data
    print("\nLoading datasets...")
    # EXPANDED EXPERIMENT: Use all sequences 00-10 with 80/20 split
    # This gives us ~19,800 samples total vs ~10,400 in the original
    all_sequences = ['00', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10']
    
    # Load all data first
    full_dataset = KITTIFullDataset(
        sequences=all_sequences, split='all',
        cache_dir='data/kitti/bev_cache_expanded'  # Separate cache!
    )
    
    # Create 80/20 train/val split from combined data
    ...
    print(f"\nExpanded dataset: {total_samples} total samples")
    print(f"  Train: {train_size} samples ({len(train_dataset)//args.batch_size} batches)")
    print(f"  Val: {val_size} samples ({len(val_dataset)//args.batch_size} batches)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loss < best_val:
            ...
            torch.save({
                ...
                'val_loss': val_loss,
            }, 'checkpoints/encoder_expanded_best.pth')
        ...
        torch.save({
            'epoch': epoch,
            'model_state_dict': model.state_dict(),
        }, 'checkpoints/encoder_expanded_latest.pth')
